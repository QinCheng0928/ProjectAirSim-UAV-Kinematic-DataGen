...
This dataset contains UAV trajectory episodes collected from ProjectAirSim.

Each row is one episode. The `states` field is a variable-length sequence sampled at approximately {sample_rate:.2f} Hz.
...
def export_dataset(input_path: Path, output_path: Path, jsonl_export: Path) -> None:
    ...
    write_jsonl(samples, jsonl_export / "train.jsonl")
